...
|------|----------|----------------|------------|
| 進行中（is_public = 1） | ✅ | ✅ 任何人 | ✅ |
| 已到期 | ❌（從首頁過濾） | ✅ 任何人（顯示「已截止」） | ❌ |
| 已鎖定 | ❌（從首頁過濾） | ✅ 任何人（顯示「已鎖定」） | ❌ |
| 未公開（is_public = 0） | ❌ | ✅ 任何人（仍可直接開） | ✅（若未到期且未鎖定） |

**說明：**
- 首頁 API 只回傳 `is_public = 1` 的團購；前端再過濾掉已鎖定與已到期者，因此只有進行中的公開團購會出現在首頁。
- `/buy/[slug]` 頁面無身份驗證保護，知道連結的人皆可瀏覽；到期或鎖定時頁面會顯示截止提示並隱藏報名表單。
