...
export async function GET(
  ...
  const db = getDb();
  const gb = db.prepare('SELECT * FROM group_buys WHERE id = ?').get(id) as { organizer_id: number; title: string } | undefined;
  ...
  const options = db.prepare('SELECT * FROM options WHERE group_buy_id = ? ORDER BY sort_order').all(id) as Array<{ id: number; label: string; name: string }>;
  const orders = db.prepare(
    'SELECT * FROM orders WHERE group_buy_id = ? ORDER BY submitted_at'
  ).all(id) as Array<{ id: number; participant_name: string; is_paid: number; submitted_at: string }>;
  const allItems = db.prepare(
    'SELECT * FROM order_items WHERE order_id IN (SELECT id FROM orders WHERE group_buy_id = ?)'
  ).all(id) as Array<{ order_id: number; option_id: number; quantity: number }>;
